...
const CreatePost: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const check = async () => {
      try {
        const res = await fetch(`/api/teachers/verification-status`);
        if (res.ok) {
  ...
  }, [isAuthenticated, user?.role, editId]);

  if (!isAuthenticated) {
    return (
      <div className="min-h-screen bg-background">
        <DashboardLayout>
          <main className="container mx-auto p-8 text-center">Please sign in to create posts. <Button onClick={() => router.push('/auth')}>Sign in</Button></main>
        </DashboardLayout>
        <Footer />
      </div>
    );
        ...
        <DashboardLayout>
          <main className="container mx-auto p-8">
            <div className="bg-card p-6 rounded-xl">
              ...
              <p className="mt-2">Your account must be verified. <Button variant="link" onClick={() => router.push('/teacher/onboarding')}>Go to onboarding</Button></p>
            </div>
          </main>
        </DashboardLayout>
        <Footer />
