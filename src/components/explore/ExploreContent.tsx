                ...
                <div className="mt-4 bg-card rounded-xl p-4 border border-border">
                  ...
                  <div className="grid md:grid-cols-3 gap-3 mt-3">
                    ...
                    <div className="flex items-end justify-end">
                      <Button variant="outline" onClick={() => { setGrade(null); setCity(null); setMinPrice(null); setMaxPrice(null); setMode('ANY'); setMinRating(null); setAvailabilityFilter(null); }}>Reset Filters</Button>
                    </div>
                  </div>
                </div>
              )}
                    ...
                    <div className="flex items-start justify-between mb-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <div className="flex items-center gap-2 text-sm text-muted-foreground"><span>{post.subject}</span><span>•</span><span>{post.createdAt ? new Date(post.createdAt).toLocaleString() : ''}</span></div>
                        </div>
                      </div>

                      <Badge variant="outline" className={'border-primary text-primary'}>{post.type === 'TEACHER_OFFERING' ? <GraduationCap className="w-3 h-3 mr-1" /> : <BookOpen className="w-3 h-3 mr-1" />}{post.type === 'TEACHER_OFFERING' ? 'Teacher' : 'Student Request'}</Badge>
                    </div>

                    <h3 className="text-lg font-semibold mb-2">{post.title}</h3>
                    <p className="text-muted-foreground mb-4 leading-relaxed">{post.description}</p>

                    <div className="flex flex-wrap gap-2 mb-4">{subjectTag.map((tag) => (<span key={tag} className="px-3 py-1 rounded-full bg-muted text-sm">{tag}</span>))}</div>

                    <div className="flex flex-wrap gap-4 text-sm text-muted-foreground mb-4">
                      <span className="flex items-center gap-1"><MapPin className="w-4 h-4" />{post.location?.city || 'Online'}</span>
                      {post.fee && (<span className="flex items-center gap-1"><DollarSign className="w-4 h-4" />${post.fee}/hr</span>)}
                    </div>

                    <div className="flex items-center justify-between pt-4 border-t border-border">
                      <div className="flex gap-2">
                        <Button variant="ghost" size="sm" className="gap-1.5" onClick={() => { if (isGuest) { toast({ title: 'Sign in to save', description: 'Log in to save posts to your account.'}); navigate('/auth'); return; } /* save logic */ }}><Heart className="w-4 h-4" />Save</Button>
                        <Button variant="ghost" size="sm" className="gap-1.5"><Share2 className="w-4 h-4" />Share</Button>
                      </div>
                      <Button variant={post.type === 'TEACHER_OFFERING' ? 'hero' : 'secondary'} className="gap-2" onClick={() => { if (isGuest) { toast({ title: 'Register to interact', description: 'Please register or log in to contact or respond to posts.'}); navigate('/auth'); return; } /* open inquiry or booking flow */ }}><MessageCircle className="w-4 h-4" />{post.type === 'TEACHER_OFFERING' ? 'Contact Teacher' : 'Respond'}</Button>
                    </div>
                  </article>
                );
              })}
